...
namespace LyndaCoursesDownloader.GUIDownloader
{
    public partial class MainForm : Form
    {
        ...
        private async void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            {
                extractors.Add(new Extractor(courseUrl.Trim(), quality, txtToken.Text.Trim(), checkBoxExerciseFiles.Checked));
            }
            ...
            catch (Exception ex)
            {
                ...
                progressBarExtractor.Style = ProgressBarStyle.Blocks;
                progressBarCourses.Value = 0;
            }
        ...
        private async Task<Course> ExtractCourse(Extractor extractor)
        {
            ...
            extractor.LinksExtractionFinished += Extractor_LinksExtractionFinished;
            lblCurrentExtractionOperation.Text = $"Extracting Course Download Links...";
            UC_CourseExtractorStatus.Status = CourseStatus.Running;

            progressBarExtractor.Style = ProgressBarStyle.Marquee;
            try
            {
                course = await extractor.GetCourse();
            }
            catch (TooManyRequestsException ex)
            {
                MessageBox.Show("Lynda.com refused connecting due to too many extractions of courses\nPlease try again with less courses", "Too many courses", MessageBoxButtons.OK, MessageBoxIcon.Error);
                string courses = String.Join(" -- ", txtCourseUrls.Text.Split(Environment.NewLine.ToCharArray(), StringSplitOptions.RemoveEmptyEntries));
                Log.Error(ex,"Too many requests to lynda. Courses : "+ courses);
                extractor.LinksExtractionFinished -= Extractor_LinksExtractionFinished;
                EnableControls(true);
                UC_CourseExtractorStatus.Status = CourseStatus.Failed;
                lblCurrentExtractionOperation.Text = "Course Extraction Failed";
        ...
        private void Extractor_LinksExtractionFinished()
        {
            UpdateUI(() => lblCurrentExtractionOperation.Text = "Extracting Course Subtitles...");
        }
        ...
        private async void MainForm_Load(object sender, EventArgs e)
        {
            ...
            toolTipUrls.SetToolTip(txtCourseUrls, "This is because lynda.com refuses too many course extractions");
        ...
        private void txtCourseUrls_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (txtCourseUrls.Lines.Length >= 4 && e.KeyChar == '\r' || e.KeyChar == '\n')
            {
                e.Handled = true;
            }
        }

        private void txtCourseUrls_TextChanged(object sender, EventArgs e)
        {
            if (txtCourseUrls.Lines.Length > 4)
            {
                txtCourseUrls.Text = String.Join(Environment.NewLine, txtCourseUrls.Lines.Take(4));
            }
        }
        ...
        private void btnExtractToken_Click(object sender, EventArgs e)
        {
            ...
            if (token is null)
            {
                MessageBox.Show($"No lynda token is found for {cmboxBrowser.SelectedItem.ToString().Replace("From ", "")}.\nPlease make sure that you are logged into lynda.com on the browser's default profile", "Token not found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                txtToken.Text = token;
