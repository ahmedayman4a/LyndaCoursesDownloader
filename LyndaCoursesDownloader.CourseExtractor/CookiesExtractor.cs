...
namespace LyndaCoursesDownloader.CourseExtractor
{
    class CookiesExtractor
    {
        ...
        internal List<Cookie> ReadChromeCookies() => ReadChromiumCookies(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + @"\Google\Chrome\User Data");

        internal List<Cookie> ReadEdgeCookies() => ReadChromiumCookies(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + @"\Microsoft\Edge\User Data");

        internal List<Cookie> ReadFirefoxCookies()
        {
        ...
        private List<Cookie> ReadChromiumCookies(string profilePath)
        {
            var cookies = new List<Cookie>();
            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        cookies.Add(new Cookie(reader.GetString(0), decodedValue));
                    }
                }
                conn.Close();
            }
